...
function getRejectedUsersById(id, index) {
  return fetch(`${API}/overview/rejected/${id}/${index}`).then((res) =>
    res.json()
  );
}
const RejectedUsers = () => {
  ...
  useEffect(() => {
    getRejectedUsersById(params.event_id, params.index)
  ...
  return (
    <>
      ...
      >
        ...
        <Heading mt="5%" fontSize={"19px"}>
          All Rejected Player for {name}: {`[${users.length}]`}
        </Heading>
        <Box mb="5%">
          {users &&
            users.map(({ _id, userName }) => <Text key={_id}>{userName}</Text>)}
        </Box>
        <Link
          style={{
            textDecoration: "none",
